...
namespace deVoid.Utils
{
    ...
    /// </summary>
    public interface IEventButMessage
    {
        string Hash { get; }
    }
    ...
    /// </summary>
    public class EventBusService : SingletonUnity<EventBusService>
    {
    ...
    {
        private Dictionary<Type, IEventButMessage> signals = new Dictionary<Type, IEventButMessage>();
        ...
        public SType Get<SType>() where SType : IEventButMessage, new()
        {
            Type signalType = typeof(SType);
            IEventButMessage _eventButMessage;

            if (signals.TryGetValue (signalType, out _eventButMessage)) 
            {
                return (SType)_eventButMessage;
            }
        ...
        private IEventButMessage Bind(Type signalType)
        {
            IEventButMessage _eventButMessage;
            if(signals.TryGetValue(signalType, out _eventButMessage))
            {
                UnityEngine.Debug.LogError(string.Format("Signal already registered for type {0}", signalType.ToString()));
                return _eventButMessage;
            }

            _eventButMessage = (IEventButMessage)Activator.CreateInstance(signalType);
            signals.Add(signalType, _eventButMessage);
            return _eventButMessage;
        }

        private IEventButMessage Bind<T>() where T : IEventButMessage, new()
        {
            return Bind(typeof(T));
        }

        private IEventButMessage GetSignalByHash(string signalHash)
        {
            foreach (IEventButMessage signal in signals.Values)
            {
